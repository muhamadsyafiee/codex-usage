...
GitHub counts downloads per Release asset. Run `./scripts/Get-DownloadStats.ps1` in PowerShell to list MSI downloads for every release and totals. Run `./scripts/Get-DownloadStats.ps1 -Tag v1.3.7` for one release. The report counts MSI downloads, not installs or active users. The checksum asset is reported in the all-asset total. This app collects no usage telemetry.
...
Version 1.3.7 uses a friendly black C mascot with eyes and waving hands. An opaque white backing keeps it visible on dark Windows surfaces; the bold black body remains visible on light ones. The settings control uses a gear icon. The original white C assets are retained as history.
