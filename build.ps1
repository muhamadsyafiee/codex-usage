...
& .tools/wix.exe build installer/Package.wxs -arch x64 -d "PublishDir=$PSScriptRoot/dist/app" -o dist/CodexUsageWidget-1.3.7-x64.msi
if ($LASTEXITCODE) { throw 'MSI build failed' }
(Get-FileHash dist/CodexUsageWidget-1.3.7-x64.msi -Algorithm SHA256).Hash | Set-Content -NoNewline dist/SHA256-1.3.7.txt
